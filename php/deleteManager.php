...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['manager_name'])) {
    require_once 'db.php';

    $name = $_POST['manager_name'];

    $stmt3 = $conn->prepare("SELECT 1 FROM buildings WHERE manager = ? LIMIT 1;");
    ...
    if($stmt3->num_rows === 0) {
        ...
        $stmt2 = $conn->prepare("DELETE FROM managers WHERE name = ?");
        ...
        if ($stmt2->affected_rows > 0) {
            ...
            header("Location: ../php/addManager.php?deleted=" . urlencode($name));
            exit();
        } else {
            $stmt1->close();
            $stmt2->close();
            ...
            $errorMsg = "Error deleting manager.";
            header("Location: ../php/addManager.php?error=" . urlencode($errorMsg));
            exit();
        }
    } else {
        $stmt3->close();
